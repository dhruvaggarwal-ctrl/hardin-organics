...
export default function TrackPage() {
  ...
  async function fetchStatus(isInitial = false) {
    try {
      ...
      if (r.status === 404) { setNotFound(true); if (isInitial) setLoading(false); return; }
      if (!r.ok) { if (isInitial) { setServerError(true); setLoading(false); } return; }
      ...
      // Network error
      if (isInitial) { setServerError(true); setLoading(false); }
    }
  }
  ...
  if (serverError) return (
    <div className="min-h-screen bg-[#F5F0E8] flex flex-col items-center justify-center px-4 py-16">
      <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-xl">
        ...
        <p className="text-[#6B6B6B] text-sm mb-6 leading-relaxed">
          We couldn&apos;t load your tracking info right now. Please try again — your order is safe.
        </p>
        <button
          onClick={() => { setServerError(false); setLoading(true); fetchStatus(true); }}
          className="block w-full bg-[#2D5016] text-white font-bold py-3.5 rounded-2xl hover:bg-[#3D6B20] transition-colors text-sm mb-2"
        >
          Try Again
        </button>
        <a
          href={`https://wa.me/919650595027?text=${encodeURIComponent("Hi! I need help tracking my order.")}`}
