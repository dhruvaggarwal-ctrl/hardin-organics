  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderRes = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount: PRICE, receipt: `acne_lp_${Date.now()}` }),
      });
      ...
      await new Promise<void>((resolve, reject) => {
        const rzp = new window.Razorpay({
          ...
          description: "Activated Charcoal Soap — Free Shipping",
    ...
    <div className="min-h-screen bg-[#F5F0E8] font-sans">
      ...
      <div className="max-w-xl mx-auto px-4 py-8 space-y-6">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-[#C4622D] hover:bg-[#A8522A] text-white font-bold py-5 rounded-2xl text-lg transition-all duration-200 hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-3"
        >
          {loading ? (
            <>
        ...
        <button
          onClick={handleCheckout}
          disabled={loading}
          className="w-full bg-[#C4622D] hover:bg-[#A8522A] text-white font-bold py-5 rounded-2xl text-lg transition-all duration-200 hover:shadow-xl disabled:opacity-70"
        >
          {loading ? "Opening Payment..." : "Try Hardin Charcoal Soap — Free Shipping"}
        </button>
