  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderRes = await fetch("/api/create-order", {
        ...
        body: JSON.stringify({ amount: COMBO_PRICE, receipt: `combo_lp_${Date.now()}` }),
      });
      ...
      await new Promise<void>((resolve, reject) => {
        const rzp = new window.Razorpay({
          ...
          description: "Combo Pack — Charcoal + Saffron Haldi Chandan (HARDIN10 Applied)",
          ...
          handler: async (response: { razorpay_payment_id: string; razorpay_order_id: string; razorpay_signature: string }) => {
            ...
            clearCart();
            router.push(`/order-success?paymentId=${response.razorpay_payment_id}&orderId=${response.razorpay_order_id}&offer=combo`);
            resolve();
          },
